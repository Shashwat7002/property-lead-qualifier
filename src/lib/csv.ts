...
export function toExportRows(results: QualificationResult[]): Record<string, string | number>[] {
  return results.map((result) => ({
    ...
    score: result.score,
    seller_likelihood_rank: result.sellerLikelihoodScore,
    contactability_rank: result.contactabilityScore,
    ...
    property_zip: result.raw.property_zip ?? result.raw.situs_zip ?? result.raw.zip ?? "",
    county: result.county,
    property_type: result.propertyType,
    year_built: result.yearBuilt ?? "",
    market_value: result.marketValue ?? "",
    assessed_value: result.raw.assessed_value ?? result.raw.tax_assessed_value ?? "",
    ...
    phone: result.phone,
    email: result.email,
    homestead_exemption: result.raw.homestead_exemption ?? result.raw.has_homestead ?? "",
    senior_exemption: result.raw.senior_exemption ?? result.raw.age65_exemption ?? "",
    vacancy_flag: result.raw.vacancy_flag ?? result.raw.usps_vacant ?? result.raw.vacant_property ?? "",
